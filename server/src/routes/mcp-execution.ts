...
router.post('/execute',async(req,res)=>{try{const result=await executeGovernedMcp(ctx(req),{tool:req.body?.tool,target:req.body?.target,arguments:req.body?.arguments},req.header('x-alphax-approval-id')||undefined);res.status(result.status==='approval_required'?428:200).json(result);}catch(e){res.status(403).json({error:String((e as Error).message)});}});
