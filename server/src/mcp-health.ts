...
export interface ProviderHealth { providerId: string; status: ProviderHealthStatus; lastCheckedAt: string | null; latencyMs: number | null; toolCount: number; error: string | null; }

export function initMcpHealthSchema(): void { getDb().exec(`CREATE TABLE IF NOT EXISTS mcp_provider_health (provider_id TEXT PRIMARY KEY,status TEXT NOT NULL,last_checked_at TEXT,latency_ms INTEGER,tool_count INTEGER NOT NULL DEFAULT 0,error TEXT);`); }
export function listProviderHealth(): ProviderHealth[] { return getDb().prepare(`SELECT provider_id as providerId,status,last_checked_at as lastCheckedAt,latency_ms as latencyMs,tool_count as toolCount,error FROM mcp_provider_health ORDER BY provider_id`).all() as unknown as ProviderHealth[]; }
...
export async function checkProviderHealth(ctx: SecurityContext, provider: McpProvider, adapter = new GovernedMcpAdapter()): Promise<ProviderHealth> {
  ...
  if (!provider.enabled) {
    ...
    saveHealth(health); return health;
  }
  try {
    const tools = await adapter.discover(ctx, provider);
    const latencyMs = Date.now() - started;
    const health: ProviderHealth = { providerId: provider.id, status: latencyMs > 5000 ? 'degraded' : 'healthy', lastCheckedAt: nowIso(), latencyMs, toolCount: tools.length, error: null };
    saveHealth(health); audit(ctx.actor, 'mcp.provider.health', provider.id, 'allow', health); return health;
  } catch (e) {
    ...
    saveHealth(health); audit(ctx.actor, 'mcp.provider.health', provider.id, 'deny', { error }); return health;
  }
}
