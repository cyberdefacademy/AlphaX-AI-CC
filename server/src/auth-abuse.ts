...
export function loginBlocked(key: string): number {
  ...
  if (now - new Date(row.window_started_at).getTime() > WINDOW_MS) getDb().prepare('DELETE FROM auth_login_abuse WHERE key_hash=?').run(hash);
  return 0;
}
...
export function recordLoginFailure(key: string): { locked:boolean; retryAfter:number; failures:number } {
  ...
  const withinWindow = Boolean(row && now - new Date(row.window_started_at).getTime() <= WINDOW_MS);
  ...
  if (failures >= MAX_FAILURES) {
    ...
    db.prepare(`INSERT INTO auth_login_abuse(key_hash,failures,window_started_at,locked_until) VALUES(?,?,?,?)
      ON CONFLICT(key_hash) DO UPDATE SET failures=excluded.failures,window_started_at=excluded.window_started_at,locked_until=excluded.locked_until`).run(hash, failures, withinWindow ? row!.window_started_at : nowText, lockedUntil);
    return { locked:true, retryAfter:Math.ceil(LOCK_MS/1000), failures };
  }
  db.prepare(`INSERT INTO auth_login_abuse(key_hash,failures,window_started_at,locked_until) VALUES(?,?,?,NULL)
    ON CONFLICT(key_hash) DO UPDATE SET failures=excluded.failures,window_started_at=excluded.window_started_at,locked_until=NULL`).run(hash, failures, withinWindow ? row!.window_started_at : nowText);
  return { locked:false, retryAfter:0, failures };
}
