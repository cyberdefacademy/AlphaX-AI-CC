...
export interface SecurityContext { actor: string; role: string; projectId?: string; target?: string; tool?: string; risk?: RiskLevel; }
const RISK_ORDER: Record<RiskLevel, number> = { low: 0, medium: 1, high: 2, critical: 3 };

export function initSecuritySchema(): void {
  const db = getDb();
  db.exec(`
    CREATE TABLE IF NOT EXISTS security_roles (id TEXT PRIMARY KEY, name TEXT UNIQUE NOT NULL, created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS security_permissions (id TEXT PRIMARY KEY, name TEXT UNIQUE NOT NULL, created_at TEXT NOT NULL);
  ...
  if (roleCount === 0) {
    ...
    for (const role of ['admin', 'security-analyst', 'pentester', 'auditor', 'viewer']) ins.run(randomId(), role, nowIso());
  }
  const permissionNames = ['security.read','missions.create','missions.execute','tools.read','tools.execute.low','tools.execute.medium','tools.execute.high','tools.execute.critical','approvals.review','audit.read','policy.manage'];
  ...
  const all = db.prepare('SELECT id, name FROM security_permissions').all() as { id: string; name: string }[];
  const rp = db.prepare('INSERT OR IGNORE INTO role_permissions (role_id, permission_id) VALUES (?, ?)');
...
export function hasPermission(role: string, permission: string): boolean {
  ...
  return Boolean(getDb().prepare(`SELECT 1 FROM security_roles r JOIN role_permissions rp ON rp.role_id=r.id JOIN security_permissions p ON p.id=rp.permission_id WHERE r.name=? AND p.name=?`).get(role, permission));
}
export function requirePermission(ctx: SecurityContext, permission: string): void { if (!hasPermission(ctx.role, permission)) throw new Error(`permission denied: ${permission}`); }
export function riskDecision(ctx: SecurityContext): Decision {
  const risk = ctx.risk ?? 'low';
  const p = getDb().prepare('SELECT max_risk,require_approval_above FROM security_policies WHERE id=? AND enabled=1').get('default') as {max_risk:RiskLevel;require_approval_above:RiskLevel}|undefined;
  if (!p) return 'deny';
  if (RISK_ORDER[risk] > RISK_ORDER[p.max_risk]) return 'deny';
...
export function createApproval(ctx: SecurityContext, request: unknown, expiresMinutes=30): string {
  ...
  audit(ctx.actor,'approval.requested',id,'approval_required',{tool:ctx.tool,target:ctx.target,risk:ctx.risk}); return id;
}
export function resolveApproval(id:string,actor:string,decision:'approved'|'denied',reason=''):void {
  const r=getDb().prepare(`UPDATE security_approvals SET status=?,decided_by=?,decision_reason=?,decided_at=? WHERE id=? AND status='pending'`).run(decision,actor,reason,nowIso(),id);
  if(Number(r.changes)!==1) throw new Error('approval not found or already resolved'); audit(actor,`approval.${decision}`,id,decision,{reason});
}
export function requireApprovedApproval(id:string,ctx:SecurityContext):void {
  const row=getDb().prepare('SELECT actor,tool,target,status,expires_at FROM security_approvals WHERE id=?').get(id) as {actor:string;tool:string;target:string|null;status:string;expires_at:string|null}|undefined;
  if(!row) throw new Error('approval not found');
  if(row.status!=='approved') throw new Error(`approval is ${row.status}`);
  if(row.expires_at && new Date(row.expires_at).getTime()<Date.now()) throw new Error('approval expired');
  if(row.actor!==ctx.actor) throw new Error('approval actor mismatch');
  if(ctx.tool && row.tool!==ctx.tool) throw new Error('approval tool mismatch');
  if(ctx.target && row.target && row.target!==ctx.target) throw new Error('approval target mismatch');
  audit(ctx.actor,'approval.consumed',id,'approved',{tool:row.tool,target:row.target});
}
export function listApprovals(status?:string):unknown[]{ return status ? getDb().prepare('SELECT * FROM security_approvals WHERE status=? ORDER BY created_at DESC').all(status) : getDb().prepare('SELECT * FROM security_approvals ORDER BY created_at DESC').all(); }
export function listAudit(limit=200):unknown[]{ return getDb().prepare('SELECT * FROM security_audit ORDER BY ts DESC LIMIT ?').all(Math.min(Math.max(limit,1),1000)); }
export function audit(actor:string,action:string,resource?:string,decision?:string,detail?:unknown):void {
  const db=getDb(); const prev=db.prepare('SELECT event_hash FROM security_audit ORDER BY ts DESC LIMIT 1').get() as {event_hash:string}|undefined; const ts=nowIso();
  const payload=JSON.stringify({ts,actor,action,resource:resource??null,decision:decision??null,detail:detail??null,previous_hash:prev?.event_hash??null}); const hash=createHash('sha256').update(payload).digest('hex');
  db.prepare(`INSERT INTO security_audit (id,ts,actor,action,resource,decision,detail,previous_hash,event_hash) VALUES (?,?,?,?,?,?,?,?,?)`).run(randomId(),ts,actor,action,resource??null,decision??null,detail?JSON.stringify(detail):null,prev?.event_hash??null,hash);
}
