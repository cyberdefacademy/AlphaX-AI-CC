...
export async function executeGovernedMcp(ctx: SecurityContext, req: ToolRequest, approvalId?: string, signal?: AbortSignal) {
  ...
  const health: any = getDb().prepare('SELECT status,last_checked_at as lastCheckedAt FROM mcp_provider_health WHERE provider_id=?').get(row.id);
  ...
  try {
    ...
    finishReceipt(ctx, receiptId, result.ok ? 'completed' : /abort|timeout/i.test(result.error || '') ? 'timeout' : 'failed', normalized ?? result.result, result.error);
    return { status: result.ok ? 'completed' : /abort|timeout/i.test(result.error || '') ? 'timeout' : 'failed', callId, receiptId, ...result, normalized };
  } catch (e) {
